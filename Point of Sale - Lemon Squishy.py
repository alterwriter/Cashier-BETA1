...
class Register:
    def __init__(self, parent):
        self.parent = parent
        self.parent.configure(background= 'gray12')
        parent.title('Afi Point of Sale')
        self.font = ('Courier New', 12)
        self.till = 0
        self.TAX = 0.08
        self.items = {'hot_chocolate':Item('Hot Chocolate', 50,
                                      tk.Button(root,
                                      text='Hot Chocolate',
                                      command=lambda: self.scan('hot_chocolate'),
                                      font=self.font)),
        ...
        self.server_label = tk.Label(root, text='Cashier: Cheysan', font=self.font)
        ...
        for i,item in enumerate(self.items.values(), start=1):
            item.button.grid(row=i, column=0, sticky='W')
        self.frame = tk.Frame(root)
        self.frame.grid(row=1, column=1, sticky='W', rowspan=i+1, columnspan=4)
        ...
        self.checkout_button.grid(row=i+2, column=1, sticky='W')
        self.till_button = tk.Button(root, text='Till', command=self.check_till)
        self.till_button.grid(row=i+2, column=2, sticky='W')
        self.new_order_button = tk.Button(root, text='New order', command=self.new_order)
        self.new_order_button.grid(row=i+2, column=3, sticky='W')
        self.exit = tk.Button(root, text='Exit', command=self.iExit)
        self.exit.grid(row=i+2, column=4, sticky='W')
        self.total_label = tk.Label(root, text='', font=self.font)
        self.total_label.grid(row=i+2, column=4, sticky='E')
        self.new_order()
        self.tick()
    def modify_item(self, event=None):
        ...
        def set_new_quantity():
            ...
            i = self.box.index(tk.ACTIVE)
            self.box.delete(i)
            code = self.current_codes.pop(i)
    ...
    def iExit(self):
        qExit = messagebox.askyesno("COFFEE TOWN", "APAKAH ANDA INGIN KELUAR?")
        if qExit > 0:
            self.parent.destroy()
            return  

def on_close():
	tk.messagebox.showinfo('Warning!!!', "do you want to exit ?")
	root.destroy()
